...
class CrudModelCommand extends GeneratorCommand
{
  ...
  */
  protected function buildClass($name)
  {
    ...
    if ($this->option('view-path')) {
      $modelPath = '_' . $this->option('view-path');
    } else {
      $modelPath = "";
    }
    ...
    ->replaceModelPath($stub, $modelPath)
  ...
  */
  ...
  protected function replacemodelPath(&$stub, $modelPath)
  {
  ...
  */
  ...
  protected function createRelationshipFunction(&$stub, $relationshipName, $relationshipType, $argsString) {
    ...
    foreach ($foreignKeys as $fk) {
      $line = trim($fk);
      $parts = explode('#', $line);
      $schemaFields = trim($parts[0]);
    }

    $tabIndent = '    ';
    $code = "public function " . $relationshipName . "() { \n" . $tabIndent
      . "return \$this->" . $relationshipType . "($argsString::class, '$schemaFields');"
      . "\n  }";

      $str = '{{relationships}}';
      $stub = str_replace($str, $code . "" . $tabIndent . $str, $stub);
