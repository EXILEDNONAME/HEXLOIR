...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    {
        return $rootNamespace . '\\' . ($this->option('controller-namespace') ? $this->option('controller-namespace') : 'Http\Controllers\Backend\__Main');
    }
    ...
    protected function buildClass($name)
    {
        ...
        $ControllerPath = $this->option('model-namespace') ? '\\' . $this->option('model-namespace') : '\\';
        $ControllerPathLower = $this->option('view-path') ? "." . strtolower($this->option('view-path')) : '';
        $ControllerPathLowerURL = $this->option('view-path') ? "/" . strtolower($this->option('view-path')) : '';
...
EOD;
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replaceControllerPathLower($stub, $ControllerPathLower)
            ->replaceControllerPathLowerURL($stub, $ControllerPathLowerURL)
    ...
    protected function replaceControllerPath(&$stub, $modelName)
    {
        $stub = str_replace('{{ControllerPath}}', $modelName, $stub);

        return $this;
    }

    protected function replaceControllerPathLower(&$stub, $modelName)
    {
        $stub = str_replace('{{ControllerPathLower}}', $modelName, $stub);

        return $this;
    }

    protected function replaceControllerPathLowerURL(&$stub, $modelName)
    {
        $stub = str_replace('{{ControllerPathLowerURL}}', $modelName, $stub);
